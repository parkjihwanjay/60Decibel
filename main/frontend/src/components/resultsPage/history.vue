<template>
  <div class="cont">
    ...
    <div class="main">
      ...
      <p v-if="`${profile.taking_medicine}`">
        {{ profile.what_medicine }}을 복용중입니다.
      </p>
      <p>
        <span class="s">{{ family_history }}</span
        >와(과) 같은 가족력이 있습니다.
      </p>
      <p v-if="`${profile.drinking}`">
        매주 {{ profile.drinking_per_week }}병의 술을 마십니다.
      </p>
      <p v-if="`${profile.smoking}`">
        담배를 {{ profile.how_long_smoking }}년 동안
        {{ profile.how_long_smoking }}갑씩 피우고 있습니다.
      </p>
      <p v-if="`${profile.relevant_data}`">
        기타 특이사항 : {{ relevant_data }}
      </p>
    </div>
  </div>
</template>
